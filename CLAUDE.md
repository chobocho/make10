...
- 삭제 후 **중력 적용** — 위에 있던 블럭이 아래의 빈 칸을 채우며 낙하 (Bejeweled 스타일)
- 중력 적용 후 **리필** — 상단의 빈 칸이 새 임의 블럭(1~9)으로 채워져 보드는 항상 가득 찬 상태를 유지
- 리필 결과에서도 유효 조합이 없으면 종료 (stuck) — 확률적으로 매우 드묾
...
### 2-6. 게임 종료 조건
- **시간 초과**: 타이머가 0에 도달 (timeup) — 주 종료 경로
- **진행 불가**: 리필 후에도 유효 조합이 없음 (stuck) — 확률적으로 드뭄
- 리필로 보드가 항상 가득하므로 "모든 셀 비움(cleared)"은 실질적으로 발생하지 않음
- `targetScore` 점수 달성 시 클리어 (추후 확장용)

### 2-7. 점수 & 별점
- 2개 삭제: +100점
- 3개 삭제: +250점
- 별점(★ 3단계): 맵별 `starThresholds: [s1, s2, s3]` (오름차순) 기준으로 최종 점수에 대응.
  - `score >= s3` → ★★★
  - `score >= s2` → ★★
  - `score >= s1` → ★ (= 성공)
  - 그 외 → 실패
- 게임 시작 시 인트로 오버레이로 3개 임계값을 표시 (2.5초 또는 탭으로 스킵, 이후 타이머 시작).
