...
- **베이스 점수 = 셀 값들의 곱 × 매치 크기 배수**
  - 2셀: `값1 × 값2 × 5` (예: 4+6 → 4×6×5 = **120**)
  - 3셀: `값1 × 값2 × 값3 × 12` (예: 3+3+4 → 3×3×4×12 = **432**)
  - **만능(?) 블럭은 곱셈에 5 로 참여** (1~9 의 기대값).
- **연쇄(chain) 보너스**: 직전 매치로부터 **1초 이내**에 다음 매치를 성공하면 누적 보너스 적용.
  - 2연쇄 +50 / 3연쇄 +100 / 4연쇄 +150 / 5연쇄 +200 / **6연쇄 이상 +250 (cap)**.
  - 최종 점수 = `(베이스 + 연쇄 보너스) × (보너스 블럭 적중 시 2, 아니면 1)`.
...
매치 시 셀 제거에 시각 피드백:
- 베이스: 방사형 파티클(중력+마찰) + 실제 획득 점수(`+값`) 팝업.
...
- **점수**: `(베이스 + 연쇄 보너스) × 2`. 예) 3연쇄(2셀) 4+6 + 보너스 적중 = `(120 + 100) × 2 = 440`.
